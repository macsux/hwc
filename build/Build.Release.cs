...
partial class Build : IGitHub, INuget, IVersion
{
    ...
    Target BeforeRelease => _ => _
        .Unlisted()
        .Executes(() =>
        {
            var nuget = this as INuget;
            var github = this as IGitHub;
            nuget.NugetFeedsToPublish = new List<INuget.NugetFeedInfo>()
            {
                new(github.GitHubNugetPackageFeedUrl, github.GitHubToken)
            };
        });
    ...
    Target Release => _ => _
        ...
        .Executes(() =>
        {
            ...
            // PublishToNugetFeed(NugetPrivateFeedUrl, NugetPrivateFeedApiKey);
            // if (GitVersion.PublicRelease)
            // {
            //     PublishToNugetFeed(NugetPublicFeedUrl, NugetPublicFeedApiKey);
            // }
        });
